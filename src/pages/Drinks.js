...
//   if (!drinks.drinks && !randomCocktails) return <div>Loading</div>;
...
//           <>
//             <section className={`rand${index + 1} randy`}>
//               <Link to={`/drinks/${cocktail._id}`}>
//                 <div key={cocktail._id} className={`idxCard`}>
//                   <h1 className="drinkName">{cocktail.name}</h1>
//                 </div>
//               </Link>
//             </section>
//           </>
...
//         <p>
//           Special thanks to CocktailsDB @{" "}
//           <span className="tyLink">
//             <a className="tyLink" href="https://www.thecocktaildb.com/">
...
const Drinks = (props) => {
  ...
  useEffect(() => {
    const fetchIngredient2 = async () => {
      try {
        const response = await fetch(
          `https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${drinks.drinks[index2].idDrink}`
        );
  ...
  useEffect(() => {
    const fetchIngredient3 = async () => {
      try {
        const response = await fetch(
          `https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${drinks.drinks[index3].idDrink}`
        );
  ...
  useEffect(() => {
    const fetchIngredient4 = async () => {
      try {
        const response = await fetch(
          `https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${drinks.drinks[index4].idDrink}`
        );
  ...
  useEffect(() => {
    const fetchIngredient5 = async () => {
      try {
        const response = await fetch(
          `https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${drinks.drinks[index5].idDrink}`
        );
        ...
        <section className="rand1 randy">
          <Link to={`/drinks/${index1}`}>
            <div key={index1} className={`randIndex1 idxCard`}>
              ...
              <img
                src={`${ingredient1.drinks[0].strDrinkThumb}`}
                alt={`${ingredient1.drinks[0].strDrink}`}
                className="indexImg indexImg1"
              />
              {ingredient1 &&
                Object.keys(ingredient1.drinks[0]).map((key, index1) => {
                  // console.log(ingredient1.drinks[0])
                  return (
                    <div key={index1} className={`ingredient_${index1} recipe`}>
                      <h3>
                        {key.startsWith("strIngredient")
                          ? ingredient1.drinks[0][key]
                          : null}
                      </h3>
        ...
        <section className="rand2 randy">
          <Link to={`/drinks/${index2}`}>
            <div key={index2} className={`randIndex2 idxCard`}>
              ...
              <img
                src={`${ingredient2.drinks[0].strDrinkThumb}`}
                alt={`${ingredient2.drinks[0].strDrink}`}
                className="indexImg"
              />
              {ingredient2 &&
                Object.keys(ingredient2.drinks[0]).map((key, index2) => {
                  // console.log(ingredient2.drinks[0])
                  return (
                    <div key={index2} className={`ingredient_${index2} recipe`}>
                      <h3>
                        {key.startsWith("strIngredient")
                          ? ingredient2.drinks[0][key]
                          : null}
                      </h3>
        ...
        <section className="rand3 randy">
          <Link to={`/drinks/${index3}`}>
            <div key={index3} className={`randIndex3 idxCard`}>
              ...
              <img
                src={`${ingredient3.drinks[0].strDrinkThumb}`}
                alt={`${ingredient3.drinks[0].strDrink}`}
                className="indexImg"
              />
              {ingredient3 &&
                Object.keys(ingredient3.drinks[0]).map((key, index3) => {
                  // console.log(ingredient3.drinks[0])
                  return (
                    <div key={index3} className={`ingredient_${index3} recipe`}>
                      <h3>
                        {key.startsWith("strIngredient")
                          ? ingredient3.drinks[0][key]
                          : null}
                      </h3>
        ...
        <section className="rand4 randy">
          <Link to={`/drinks/${index4}`}>
            <div key={index4} className={`randIndex4 idxCard`}>
              ...
              <img
                src={`${ingredient4.drinks[0].strDrinkThumb}`}
                alt={`${ingredient4.drinks[0].strDrink}`}
                className="indexImg"
              />
              {ingredient4 &&
                Object.keys(ingredient4.drinks[0]).map((key, index4) => {
                  // console.log(ingredient4.drinks[0])
                  return (
                    <div key={index4} className={`ingredient_${index4} recipe`}>
                      <h3>
                        {key.startsWith("strIngredient")
                          ? ingredient4.drinks[0][key]
                          : null}
                      </h3>
        ...
        <section className="rand5 randy">
          <Link to={`/drinks/${index5}`}>
            <div key={index5} className={`randIndex5 idxCard`}>
              ...
              <img
                src={`${ingredient5.drinks[0].strDrinkThumb}`}
                alt={`${ingredient5.drinks[0].strDrink}`}
                className="indexImg"
              />
              {ingredient5 &&
                Object.keys(ingredient5.drinks[0]).map((key, index5) => {
                  // console.log(ingredient5.drinks[0])
                  return (
                    <div key={index5} className={`ingredient_${index5} recipe`}>
                      <h3>
                        {key.startsWith("strIngredient")
                          ? ingredient5.drinks[0][key]
                          : null}
                      </h3>
